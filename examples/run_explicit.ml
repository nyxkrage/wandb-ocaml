...
let main () =
  ...
  in
  let* run = Wandb.Run.init
    ~project:"ocaml-api-smoke-test"
    ~name:"ocaml-explicit-handle"
    ~notes:"Run managed via explicit handle."
    ~tags:[ "ocaml"; "explicit" ]
    ~config
    ()
  in
  let result = train run in
  match result with
  | Ok () -> Wandb.Run.finish run
  | Error _ -> let _ = Wandb.Run.finish ~exit_code:1 run in result

let () =
  match main () with
  | Ok () ->
      Printf.printf "Finished explicit-handle training run.\n%!"
  | Error err ->
      Printf.eprintf "Explicit-handle run failed: %s\n%!" (Wandb.Error.to_string err);
      exit 1
